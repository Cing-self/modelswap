...
describe('sync pull reconciles every registered Agent through local discovery', { timeout: 30000 }, () => {
  it('hydrates B-local models and writes every native Agent config from the dynamic registry', async () => {
    ...
    const common = String.raw`
      ...
      const provider={
        id:'sync-all-adapters', name:'Sync all adapters', type:'openai', baseUrl:'${origin}/open/v1', authMode:'api_key', vaultKey:'SYNC_ALL_ADAPTERS_KEY',
        endpoints:[
    ...
    const push = `${common}
      (async()=>{
        ...
        const vault=new VaultStore(); await vault.set('SYNC_ALL_ADAPTERS_KEY','sync-all-secret');
    ...
    const pull = `${common}
      (async()=>{
        ...
        const files={
          claude:(Boolean(claude.env.ANTHROPIC_API_KEY)||Boolean(claude.apiKeyHelper))&&claude.env.ANTHROPIC_BASE_URL==='${origin}/anthropic'&&claude.env.ANTHROPIC_MODEL===remote&&claude.env.ANTHROPIC_DEFAULT_SONNET_MODEL===remote,
          codex:codex.includes('model = "'+remote+'"')&&codex.includes('base_url = "${origin}/open/v1"')&&codex.includes('SYNC_ALL_ADAPTERS_KEY')&&!codex.includes('requires_openai_auth'),
          opencode:Boolean(opencode.provider['sync-all-adapters'].options.apiKey)&&opencode.provider['sync-all-adapters'].options.baseURL==='${origin}/open/v1'&&Boolean(opencode.provider['sync-all-adapters'].models[remote]),
          openclaw:Boolean(openclaw.models.providers['sync-all-adapters'].apiKey)&&openclaw.models.providers['sync-all-adapters'].baseUrl==='${origin}/open/v1'&&openclaw.agents.defaults.model.primary==='sync-all-adapters/'+remote,
          workbuddy:Array.isArray(workbuddy)&&workbuddy.some(item=>item.id===remote&&item.url==='${origin}/open/v1/chat/completions'&&Boolean(item.apiKey)),
          zcode:Boolean(zcode.provider['sync-all-adapters'].options.apiKey)&&zcode.provider['sync-all-adapters'].options.baseURL==='${origin}/open/v1'&&Boolean(zcode.provider['sync-all-adapters'].models[remote])&&zcodeCli.modelCatalog.overrides['sync-all-adapters/'+remote].supportsImages===false,
          hermes:hermes.includes('api: ${origin}/open/v1')&&hermes.includes('default_model: '+remote)&&hermes.includes('api_key:'),
          'kimi-code':kimi.includes('base_url = "${origin}/open/v1"')&&kimi.includes('model = "'+remote+'"')&&kimi.includes('api_key ='),
          grok:grok.includes('base_url = "${origin}/open/v1"')&&grok.includes('model = "'+remote+'"')&&grok.includes('api_key ='),
          'mimo-code':Boolean(mimo.provider['sync-all-adapters'].options.apiKey)&&mimo.provider['sync-all-adapters'].options.baseURL==='${origin}/open/v1'&&Boolean(mimo.provider['sync-all-adapters'].models[remote]),
        };
        await new Promise(resolve=>server.close(resolve));
        console.log(JSON.stringify({metaIds:AGENTS_META.map(a=>a.id),registryIds:getAdapters().map(a=>a.id),result,files,cacheIds:(cache.providers[provider.id]||[]).map(m=>m.id),providersHasRebuildable:/"models"\s*:|"platforms"\s*:|"modelCache"\s*:/.test(providersText)}));
      })().catch(error=>{console.error(error.stack||error);process.exit(1)});
    `;
