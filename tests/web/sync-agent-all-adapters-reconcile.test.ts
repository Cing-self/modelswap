  ...
  it('hydrates B-local models and writes every native Agent config from the dynamic registry', async () => {
    ...
    const push = `${common}
      (async()=>{
        ...
        await sync.updateSettingsSync(state.sync);
        for(const [agentId,selection] of Object.entries(state.agentProviders)) await sync.applyAgentBinding(agentId,selection);
