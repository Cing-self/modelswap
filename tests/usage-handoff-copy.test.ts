...
describe('usage hand-off i18n contract', () => {
  it('renders MiMo, OpenCode Go, GitHub Copilot, and every policy category in the selected language', () => {
    const cases = [
      ...
      { name: 'plugin connection refresh', usage: usageFrom(handoff.pluginRefreshHandoff('mimoConsole')), zh: ['请连接 OKIT 浏览器插件后刷新。'], en: ['Connect the OKIT browser extension, then refresh.'], refresh: true },
