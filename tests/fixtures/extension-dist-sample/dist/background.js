...
async function findServerPort() {
    for (const port of MODELSWAP_PORTS) {
        try {
            ...
            if (res.ok)
                return port; // unexpected responses fall through to the next port
        }
        catch {
...
function scheduleReconnect() {
    ...
    if (reconnectAttempts > MAX_EAGER_ATTEMPTS)
        return; // let keepalive alarm handle it
 ...
 */
async function getAutomationWindow(initialUrl) {
...
async function resolveTabId(explicitTabId, initialUrl) {
    ...
    if (!newTab.id)
        throw new Error('Failed to create tab in automation window');
...
const stealthInjectedTabs = new Set();
async function ensureStealthInjected(tabId) {
    if (stealthInjectedTabs.has(tabId))
        return;
...
const clipboardReadPending = new Map();
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    if (msg?.type === 'modelswap-clipboard-read-result' && typeof msg.requestId === 'string') {
        ...
        if (typeof msg.error === 'string')
            pending.reject(new Error(`Clipboard read failed: ${msg.error}`));
        else if (typeof msg.text === 'string')
            pending.resolve(msg.text);
        else
            pending.reject(new Error('Clipboard read returned no text'));
        return false;
    }
...
let vaultRequests = [];
async function persistVaultRequests() {
 ...
 */
async function injectCopyGuardScripts(tabId) {
    ...
    await chrome.scripting.executeScript({ target: { tabId }, files: ['dist/copy-guard-main.js'], world: 'MAIN' });
}
function pendingRequestDomains() {
    const now = Date.now();
    ...
    for (const req of vaultRequests) {
        ...
        for (const item of req.items) {
            ...
            try {
                domains.add(registrableDomain(new URL(item.url).hostname));
            }
            catch { /* bad url */ }
        }
    }
...
async function injectCopyGuardIntoMatchingTabs() {
    const { domains, coversAnyPage } = pendingRequestDomains();
    if (domains.size === 0 && !coversAnyPage)
        return;
    ...
    for (const tab of tabs) {
        ...
        if (!domains.has(registrableDomain(host)) && !coversAnyPage)
            continue;
        try {
...
chrome.tabs.onUpdated.addListener(async (tabId, info, tab) => {
    ...
    if (!host || !domains.has(registrableDomain(host)))
        return;
    try {
...
function updateVaultBadge() {
    const now = Date.now();
    const pending = vaultRequests.reduce((count, req) => (req.expiresAt > now ? count + req.items.filter(i => i.status !== 'fulfilled').length : count), 0);
...
async function loadVaultRequests() {
    try {
        ...
        vaultRequests = st.vaultRequests ?? [];
    }
    catch {
        vaultRequests = [];
    }
...
function resolveCaptureResult(msg) {
...
function sendCapture(requestId, item, payload, confirmed, source) {
...
async function completeItem(requestId, key, result) {
    ...
    notifyBasic(`${key} 已存入 MODELSWAP ✅${result.duplicate ? '（与现有值相同）' : ''}`, preview ? `值: ${preview}` : '');
}
const confirmDecisions = new Map();
function askConfirm(requestId, item, payload, source, note) {
...
chrome.notifications.onButtonClicked.addListener((notificationId, buttonIndex) => {
    ...
    if (buttonIndex !== 0)
        return; // "不是这个" — plain dismiss
    void (async () => {
        const result = await sendCapture(decision.requestId, decision.item, decision.payload, true, decision.source);
        if (result.ok)
            await completeItem(decision.requestId, decision.item.key, result);
        else
            notifyBasic('保存失败', result.error ?? '未知错误');
    })();
});
...
function isPatternSafe(pattern) {
    ...
    while (i < source.length) {
                ...
                if (group.branches && group.branches.length > 1) {
                    for (let b = 0; b < group.branches.length; b++) {
                        for (let c = b + 1; c < group.branches.length; c++) {
                            ...
                            if (fb && fc && typeof fb === "string" && typeof fc === "string" && fb[0] === fc[0])
                                return false;
                        }
                    }
                }
                if (parent)
                    parent.unboundedInside = true;
            }
        ...
        if (branch && branch[branch.length - 1] === 0)
            branch[branch.length - 1] = ch;
        i += 1;
    }
...
function looksSecret(s) {
    if (/\s/.test(s) || s.length < 20 || !/^[A-Za-z0-9_\-.=+/]+$/.test(s))
        return false;
    const freq = {};
    for (const ch of s)
        freq[ch] = (freq[ch] ?? 0) + 1;
...
function matchItem(item, text, pageUrl) {
    ...
    if (item.fields?.length) {
        ...
        for (const n of present) {
            ...
            if (re && !re.test(parsed[n]))
                patternsOk = false;
        }
        const fields = {};
        for (const n of present)
            fields[n] = parsed[n];
        if (all && patternsOk && domain)
            return { tier: 'auto', payload: { fields }, note: '' };
    ...
    if (re) {
        ...
        if (patternOk && domain)
            return { tier: 'auto', payload: { value: single }, note: '' };
        if (patternOk)
            return { tier: 'confirm', payload: { value: single }, note: '复制来源不是预期控制台域名' };
        if (domain && looksSecret(single))
            return { tier: 'confirm', payload: { value: single }, note: '与 agent 预期格式不符' };
        return null;
    }
    // No pattern supplied — heuristic-only, always confirm tier.
    if (domain && looksSecret(single))
        return { tier: 'confirm', payload: { value: single }, note: '' };
    if (!item.url && looksSecret(single))
        return { tier: 'confirm', payload: { value: single }, note: '' };
    return null;
}
async function handleCopyDetected(msg) {
    if (vaultRequests.length === 0)
        return;
    const now = Date.now();
    let auto = null;
    ...
    for (const req of ordered) {
        ...
        for (const item of req.items) {
            ...
            if (!confirmCandidate)
                confirmCandidate = { req, item, match };
        }
        if (auto)
            break;
    }
    if (auto) {
        ...
        if (result.ok) {
            ...
            askConfirm(auto.req.id, auto.item, auto.match.payload, msg, result.code === 'key-exists' ? '同名 key 已存在，确认覆盖' : '与 agent 预期格式不符');
        }
        else {
...
async function handleManualCapture(msg) {
    const now = Date.now();
    for (const req of vaultRequests) {
        if (req.expiresAt <= now)
            continue;
...
async function handleNavigate(cmd) {
    if (!cmd.url)
        return { id: cmd.id, ok: false, error: 'Missing url' };
    ...
    await new Promise((resolve) => {
        ...
        checkTimer = setTimeout(async () => {
            try {
                ...
                if (t.status === 'complete' && (t.url === targetUrl || t.url !== beforeUrl))
                    finish();
            }
            catch { /* tab gone */ }
        }, 100);
...
async function handleTabs(cmd) {
    switch (cmd.op) {
        ...
        case 'close': {
            ...
            if (tabId === null)
                return { id: cmd.id, ok: false, error: 'No tab to close' };
...
async function handleFocusWindow(cmd) {
    if (automationWindowId === null) {
        return { id: cmd.id, ok: false, error: 'No automation window' };
    }
    try {
...
async function handleCloseWindow(_cmd) {
    if (automationWindowId !== null) {
        try {
            await chrome.windows.remove(automationWindowId);
        }
...
async function handleSetFileInput(cmd) {
...
async function handleInsertText(cmd) {
...
async function handleNetworkCaptureStart(cmd) {
    const tabId = await resolveTabId(cmd.tabId);
    try {
        await ensureStealthInjected(tabId);
...
async function handleNetworkCaptureRead(cmd) {
    const tabId = await resolveTabId(cmd.tabId);
    try {
 ...
 */
async function handleClipboardRead(cmd) {
    ...
    try {
        ...
        let matcher;
        try {
            matcher = new RegExp(cmd.clipboardPattern);
        }
        catch {
