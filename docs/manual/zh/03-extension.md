...
**桌面应用用户**：扩展同样内置在 app 里。打开 **设置 → 浏览器插件 → 打开扩展目录**，扩展会被同步到 `~/.modelswap/extension` 并在访达中打开——无需安装 CLI。
...
2. 右上角开启**开发者模式**
3. 点击**加载已解压的扩展程序**
4. 选择 `manifest.json` 所在的**扩展根目录**——npm 安装的用户选 `modelswap extension path` 的输出，源码用户选仓库的 `extension/` 目录（构建出 `dist/` 之后）。注意不是其中的 `dist` 子目录。
