...
模型管控页内置 42 个 Provider 预设（官方 API、聚合平台、国内外云厂商），也支持添加自定义平台。
...
- **端点**：可配置多个，每行选协议类型（anthropic / openai / responses / typesafe）+ Base URL；自定义平台可在「高级协议设置」里把 openai 端点在 chat / responses 之间切换
