...
**桌面版（推荐）：** 从 [GitHub Release](https://github.com/Cing-self/modelswap/releases/latest) 下载 dmg，拖入「应用程序」并打开——控制台窗口会自动出现。首次打开如被 macOS 拦截，按[第 2 章](02-install.md)的步骤放行一次。
...
更多安装方式（源码构建等）见[第 2 章](02-install.md)。
...
然后找 Agent 问个小问题。如果它还在用旧模型，说明正在运行的 CLI 会话缓存了旧配置——重启该 Agent（见 [常见问题](15-faq.md)）。

到这里你已经跑通了核心流程。接下来可以按需深入：

- **密钥管理**：批量创建密钥（[第 4–5 章](04-extension.md)）
- **用量监控**：订阅余额查询与告警（[第 8 章](08-usage.md)）
- **多设备同步**：iCloud / WebDAV / Cloudflare 等端到端加密同步（[第 11 章](11-sync.md)）
- **更多 Agent**：查看全部支持的 Agent 与切换方式（[第 9 章](09-agents.md)）
