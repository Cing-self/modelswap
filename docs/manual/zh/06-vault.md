...
`modelswap hook` 命令已在 v1.0.3 移除，当前版本没有 cd 钩子功能。如需进入项目自动注入密钥，用 `modelswap vault inject` 配合你的 shell 工具（direnv 等）自行组合；ModelSwap 本身**永远不会**修改你的 Shell 配置（`~/.zshrc` / `~/.bashrc` 等）。旧版本装过 hook 的清理办法见第 15 章 FAQ。
