...
| `auth-verify` | 专用持久目录 `~/.modelswap/provider-live-acceptance/profiles/auth`（用户自行登录） | 到达已登录控制台、找到预期安全入口文案/订阅掩码 Key | 点击创建/确认/生成/删除、代输账号密码验证码 |
...
1. **专用 profile 隔离**。Chrome 永远以 `--user-data-dir=<验收根目录内路径>` 启动；`assertSafeProfileDir` 双重校验（必须在 `~/.modelswap/provider-live-acceptance/` 内 + 不得与三平台日常浏览器目录重叠/包含）。`--profile` 只接受简单标识符（不是路径），结构上无法指向日常 Chrome；`--user-data-dir`、`--copy-profile`、`--import`、`--load-cookies` 等参数直接拒绝。工具绝不读取/复制/备份/导出日常 Chrome 的 Cookie、LocalStorage、IndexedDB、钥匙串或配置。
2. **guest/auth-verify 无创建通路**。两模式不加载 ModelSwap 扩展（避免与日常 ModelSwap 服务争抢单一扩展连接——扩展按 3780→3785 顺序探测锁定），改用 CDP 直连专用 Chrome 的调试端口（默认 9333），驱动只实现只读原子（open-tab/probe/screenshot/close-tab/dispose）并经 `assertDriverActionAllowed` 白名单闸门——代码结构上不存在 click/type/submit。
...
4. **产物与脱敏**。报告/截图只写 `~/.modelswap/provider-live-acceptance/`（不进 Git）。所有页面来源字符串经 `redactSecrets`（sk-/xai-/tp-/bce-v3/AKLT/JWT/Bearer/32+hex → `[REDACTED]`），URL 经 `sanitizeUrl`（去 query/fragment/credentials，非 http(s) scheme 整体 `[REDACTED]`）。截图策略默认 guest=all、auth-verify=login-only（已登录健康控制台不截，避免账号 UI 入图）。
5. **create-cleanup 会话绑定（P0）**。`/api/vault/cdp-status` 的 available=true 只证明“某个扩展在线”，不能证明它在专用 Chrome 里——产品扩展的 WebSocket hello 只带 version/protocol，日常 Chrome 的扩展同样会连上服务端。因此真实运行要求**可验证的会话证明**，缺一即拒绝（`unverified_extension_identity`，exit 1，绝不降级为“提示先停用日常扩展”）：
   ...
   - **未消除的 P0 → 真实执行硬禁用（裁定 2026-08-31）**：服务端是单扩展槽位（后认证的扩展会替换 `extWs`），witness 只能证明专用扩展“刚刚在线”；日常 Chrome 扩展随后重新认证即可完成替换，而自动创建命令不按 session 绑定——在不改产品扩展/服务端协议的前提下，同机无法证明真实创建永远不落到日常 Chrome。因此 create-cleanup 的真实执行**即便三重门槛+身份证明全部通过也直接拒绝**（`disabled`，exit 1，零委托、零网络），仅 dry-run 可用；不作为可执行真实验收能力宣传。解禁二选一：① 在隔离 VM/独立机器运行专用 ModelSwap 服务与专用 Chrome（不改产品协议的安全方案）；② 改产品 WS 协议——服务端记录 `acceptanceSession`、每次自动创建命令按 session 发送、连接被替换后立即拒绝（同机运行的正确方案）。
6. **信号清理（P1）**。guest/auth-verify 运行中收到 SIGINT/SIGTERM 时，尽力（3 秒预算）关闭本次启动的专用 Chrome 并删除 guest 临时 profile，然后以 130/143 退出。**SIGKILL 无法捕获**——强杀会遗留 `~/.modelswap/provider-live-acceptance/tmp/guest-<stamp>/` 目录与 Chrome 进程；下次 guest 用全新 stamp 目录互不影响，残留可 `rm -rf ~/.modelswap/provider-live-acceptance/tmp` 手工清理。
...
## 报告 schema（`~/.modelswap/provider-live-acceptance/reports/<stamp>-live-<mode>.json`）
...
1. ModelSwap 服务在跑且有扩展在线（`GET /api/vault/cdp-status` → `available=true`；必要条件，不是放行条件）。
...
5. Cloudflare 走 API 模式，需 `MODELSWAP_AUTOCHECK_CLOUDFLARE_PARENT_TOKEN` 专用父 Token（不得复用生产 Vault Token）。

**解禁二选一**（落地方案评审通过后，移除 orchestrate 中 `createCleanupRealRunEnabled=false` 的硬禁用分支并更新本文档）：

- **隔离方案**：在隔离 VM / 独立机器中运行专用 ModelSwap 服务与专用 Chrome——物理上不存在日常 Chrome 扩展竞争同一 WS 槽位，不需要改产品协议。
