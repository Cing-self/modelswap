...
### 纪律二点五：密钥不在库里，唯一路径是发捕获请求

不要自行获取密钥值再存入——浏览器读值、node 子进程管道等一切"自取"都是绕过。明文只允许存在于「用户剪贴板 → 扩展 → vault」这一条路径：不进对话（字面红线），也不进你控制的浏览器内核、子进程等任何 agent 自控环境（架构本意）。仅当用户亲自把值交给你时才走 `vault set --stdin` 降级路径。
