...
走查时要同步产出**选择器**（步骤语法 `显示文本@@CSS选择器`，扩展在页面上聚光灯高亮该元素）。选择器的写法与验证：

- **优先稳定锚点**：`aria-label`、`data-*`、语义化文案（如「aria-label 为 Copy value 的按钮」）；避免纯 `nth-child` 长链——页面一改版就碎；
- **写完必须过三关**：①存在（querySelector 找得到）②唯一（querySelectorAll 恰好 1 个）③到位（读命中元素里的文字，确认就是目标）——三关全过才写进步骤；
- **失配的后果是可控的**：选择器没命中时向导自动降级为纯文字引导，不会卡死流程；入库正确性由形状门兜底，与选择器无关。所以大胆写、实测验证，不要因为怕碎而放弃引导。
...
### 纪律二点五：先判定获取方式——真实凭证走捕获，测试值可直取

发请求之前先判断你要拿的是什么，这决定唯一合法的路径：

- **真实凭证**（任何需要登录的控制台、任何有风控的平台、任何真实账号的密钥）→ **只能** vault request + 扩展捕获。明文只允许存在于「用户剪贴板 → 扩展 → vault」这一条路径——你自己控制的浏览器内核、node 子进程、任何 agent 自控环境里也**不允许出现明文**（"不进对话"是字面红线，"不进 agent 自控内存"是架构本意，两者都要守）。也不要在真实账号的页面上做自动化点击——风控会识别。
- **公开生成器或一次性测试值**（无登录、无风控、值本身不是任何真实账号的凭证，如 randomkeygen 这类）→ 可自行获取直存：`vault set --stdin` 管道传入，全程脱敏纪律同对话（扫描 DOM 只回传标签/长度，不回传值）。

判定拿不准时按真实凭证处理（走捕获），或问用户。
