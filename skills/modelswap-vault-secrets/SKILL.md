...
| 单 key 无 pattern | 该控制台上复制也自动入库（agent 指定了控制台即背书，形状不校验），弹「已存入」通知；误存删 key 重发即可 |
| 多 key 一条请求 | 复制**按顺序**自动入库：第 1 个复制的值进第 1 把待命 key，依次类推——发请求前把顺序向用户讲清楚，顺序贴错删 key 重发；无形状校验。公共步骤区和「打开控制台」按钮各显示一次，卡片只有名称和粘贴输入框 |
...
- 多把 key **操作步骤完全相同**、按顺序复制、或用户明确要一条请求 → 多 key 合并：`--step` 写一遍、全部 key 共用（扩展显示一次公共步骤区和打开控制台按钮），预告「复制按顺序自动入库：第 1 个值进第 1 把 key」；
