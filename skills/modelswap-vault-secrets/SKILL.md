...
| 多 key 一条请求 | `--step` 全部 key 共用：写一遍，CLI 复制进每个 key，每张卡片各显示一份相同步骤。每 key 一张卡片、各带输入框，把值贴进对应卡片；复制捕获走一次点击确认，无形状校验 |
...
- 多把 key **操作步骤完全相同**、或用户明确要一条请求 → 多 key 合并：`--step` 写一遍、全部 key 共用（每张卡片会重复显示同一组步骤），预告「每 key 一张卡片手动归位、无形状校验」；
