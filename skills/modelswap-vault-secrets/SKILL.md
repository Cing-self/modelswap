...
命令参数决定用户在扩展里看到的交互形态，发请求前必须预告。注意：请求待命期间（30 分钟），`--url` 指向的域名上任何 ≥8 位的复制都会自动入库——提前讲清楚，别让用户在同一站点复制无关内容。实测过页面后，步骤可以带选择器引导（`显示文本@@CSS选择器`，扩展会在页面上聚光灯高亮该元素，复制后自动进入下一步；含 Regenerate/Delete 的步骤自动挂红色危险警示）：

| 请求形态 | 用户在扩展里看到的交互 |
|---|---|
| 单 key ± `--pattern` | 该控制台上复制静默自动入库（域名+形状双中，零点击）；形状不符降级为一次点击确认 |
| 多 key 同作用 + 共享 `--pattern` | 复制**按顺序**静默自动入库：第 1 个匹配形状的值进第 1 把 key，依次类推；形状不符的复制被忽略。公共步骤区和「打开控制台」按钮各显示一次，卡片只有名称和粘贴输入框 |
| 多 key 异作用 + `--key-pattern` | 每把 key 挂自己的形状，形状即路由：复制哪个值自动进哪把 key，无顺序要求；形状不符的复制被忽略或弹一次确认 |
| 多 key 一条请求（无 pattern） | 复制按顺序自动入库但不校验形状，其余同上 |
| `--fields` 多字段 | 一个多行输入框**整块粘贴** `字段名: 值`（每行一条，不是每字段一个框），字段清单逐项显示 待复制/已捕获 |

### 纪律三：多 key 请求先按「作用」选型

判断标准是 key 的**作用**，不是数量：

- **作用相同、可互换**的 key（如同一个服务的三把同等权限 token）→ 一条多 key 请求 + 共享 `--pattern`：值按复制顺序自动入库（第 1 个匹配形状的复制进第 1 把 key，依次类推），形状不符的复制被忽略。key 可互换，顺序错乱无关紧要；
- **作用不同、形状也不同的 key**（如 `api_key` + `api_secret` 各有职责）→ 一条多 key 请求 + `--key-pattern "KEY=regex"` 给每把 key 挂各自的形状：形状即路由，复制哪个值自动进哪把 key，无顺序要求（各 key 需要不同操作步骤时才拆条，per-key 步骤尚不支持）；
- **真正单实体、永远成对使用**的凭证（如 `app_id`+`app_secret`）→ `--fields`，并预告「单框多行整块粘贴」；
- 拆条与合并都可行、或不确定用户怎么用时，**先问用户偏好再发**（已知偏好案例：一条请求 > 形状校验）。
