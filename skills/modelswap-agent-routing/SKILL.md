...
```bash
modelswap provider search glm-5          # 结果已排序：精确 id > 前缀系列 > 已认证 > 其余
```

- **有精确命中**（排序后第一梯队就是 `glm-5` 本尊）：向用户确认一句「将切换到 X 平台的 glm-5」，确认后切换。同一 modelId 出现在多个平台时，把平台候选（含认证状态、国内/国际差异）列给用户选，不替用户挑。
- **只有系列命中**（用户说的 `glm-5` 实际是系列名，存在 `glm-5-turbo` / `glm-5-flash` / `glm-5-air` 等变体）：把变体连同定位差异呈现给用户——通常 `-turbo`/`-flash` 为快速轻量档、`-air` 为轻量档、无后缀为标准档、`-thinking` 为推理档——**不要替用户推断具体档位**。
- **用户说「最新」「最好」**：不要自行挑版本。按搜索结果列出该系列全部版本（含日期后缀的通常是快照版），让用户指定。
- **完全无命中**：提醒换关键词，或走「自定义平台」接入。
