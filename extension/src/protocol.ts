...
export interface Command {
  /** Unique request ID — generated by MODELSWAP server, echoed by extension in Result */
  id: string;
  ...
  code?: string;
  /** Logical session key — MODELSWAP is single-user, so this is always 'modelswap', but kept
...
export const wsUrl = (port: number) => `ws://${MODELSWAP_HOST}:${port}/ws/extension`;
...
export const pingUrl = (port: number) => `http://${MODELSWAP_HOST}:${port}/ping`;
...
export const tokenUrl = (port: number) => `http://${MODELSWAP_HOST}:${port}/api/extension/token`;
