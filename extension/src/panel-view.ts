...
function renderItem(item: RequestItem): HTMLElement {
  ...
  if (item.fields?.length) {
    const done = new Set(
      item.masked && typeof item.masked === "object" && !Array.isArray(item.masked)
...
function renderSaveForm(): void {
  ...
  groupInput.addEventListener("keydown", (e) => {
    if (groupMenu.hidden) return;
    const opts = [...groupMenu.querySelectorAll<HTMLButtonElement>(".group-opt")];
    if (e.key === "Escape") {
      closeGroupMenu();
  ...
  document.addEventListener("pointerdown", (e) => {
    if (!groupMenu.hidden && !groupWrap.contains(e.target as Node)) closeGroupMenu();
  });
