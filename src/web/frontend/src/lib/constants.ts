...
export const PLATFORM_DOCS: Record<string, { fields: Record<string, { label: string; hint: string }>; steps: { text: string; links?: Record<string, string> }[]; code?: { title: string; sql: string } }> = {
  ...
  'cloudflare-d1': {
    ...
    steps: [
      ...
      { text: '将 Token 填入配置中，测试连接后会自动创建 modelswap-sync 数据库' },
    ],
  },
  'cloudflare-r2': {
    fields: {
  ...
  supabase: {
    ...
    code: {
      ...
      sql: `CREATE TABLE modelswap_sync (\n  id BIGSERIAL PRIMARY KEY,\n  key TEXT NOT NULL UNIQUE,\n  value JSONB NOT NULL,\n  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()\n);\n\nALTER TABLE modelswap_sync ENABLE ROW LEVEL SECURITY;\nCREATE POLICY "Allow service_role" ON modelswap_sync FOR ALL USING (true) WITH CHECK (true);`,
    },
  },
  'cloudflare-kv': {
    fields: {
    ...
    steps: [
      ...
      { text: '将 Token 填入配置中，测试连接后会自动创建 modelswap-sync 命名空间' },
    ],
  },
  webdav: {
    fields: {
    ...
    steps: [
      ...
      { text: '点击测试连接，会自动创建 modelswap-sync 目录' },
    ],
  },
  lan: {
    fields: {
    ...
    steps: [
      ...
      { text: '在另一台电脑的 MODELSWAP 设置中开启「局域网同步」，复制它显示的配对码' },
      { text: '配对码格式为 modelswap-lan://IP:端口/令牌：手动填写时地址填 http://IP:端口，令牌填斜杠后的字符串' },
  ...
  icloud: {
    ...
    steps: [
      ...
      { text: '云盘内的「桌面与文稿文件夹」等选项无需勾选——MODELSWAP 只使用 iCloud Drive 根目录的独立文件夹，不会碰你的桌面和文稿' },
      { text: '回到本页设置「同步密码」（自定义，所有机器填相同的密码），在下方平台列表打开 iCloud 的启用开关即可——所有启用的平台会同时同步，互为备份' },
      { text: '无需填任何账号密码，启用后同步数据会自动加密写入 iCloud Drive 的 modelswap-sync 文件夹（可在访达边栏「iCloud 云盘」中查看，内容为密文）' },
      { text: '建议开启「自动同步」：本地改动自动推送，远端数据定时自动合并，无需手动操作' },
      { text: '其他 Mac 登录同一 iCloud 账户、在 MODELSWAP 中输入相同同步密码并启用 iCloud，即可互相同步；Linux / Windows 用户请改用 WebDAV' },
    ],
  },
};
