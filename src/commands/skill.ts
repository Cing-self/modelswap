...
const ROUTING_HOOK_START = "<!-- modelswap-skills:start -->";
const ROUTING_HOOK_END = "<!-- modelswap-skills:end -->";

// Each agent has exactly one always-injected instructions file — that is the
// only deterministic routing surface (skill lists are not injected in
// headless modes, and weak models trigger unreliably even when they are).
function routingFileFor(agentId: string, skillsDir: string): string {
  const base = path.dirname(skillsDir);
  const fileName = agentId === "claude" ? "CLAUDE.md" : "AGENTS.md";
  return path.join(base, fileName);
}

async function ensureRoutingHook(agentId: string, skillsDir: string): Promise<void> {
  const routingFile = routingFileFor(agentId, skillsDir);
  const block = [
    ROUTING_HOOK_START,
    "## ModelSwap 技能路由",
  ...
  let existing = "";
  if (await fs.pathExists(routingFile)) existing = await fs.readFile(routingFile, "utf8");
  if (existing.includes(ROUTING_HOOK_START)) {
    // Refresh the block in place (rules may evolve between versions).
    const start = existing.indexOf(ROUTING_HOOK_START);
    const end = existing.indexOf(ROUTING_HOOK_END) + ROUTING_HOOK_END.length;
    const updated = existing.slice(0, start) + block + existing.slice(end);
    if (updated !== existing) await fs.writeFile(routingFile, updated);
    return;
  }
  const separator = existing.trim().length === 0 ? "" : "\n\n";
  await fs.writeFile(routingFile, existing + separator + block + "\n");
  console.log(kleur.green(`✓ 已在 ${routingFile} 写入 ModelSwap 技能路由(对所有模型生效)`));
}
...
export async function skillAdd(options: SkillAddOptions): Promise<void> {
  ...
  for (const agent of chosenAgents) {
    await ensureRoutingHook(agent.id, agent.skillsDir);
  }
